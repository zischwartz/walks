...
function detectPoseInRealTime(video, net) {
  ...
  async function poseDetectionFrame() {
    ...
    // added by zach
    // console.log(poses);
    let left = poses[0]["keypoints"].find(x => x["part"] == "leftShoulder");
    // let left = poses[0]["keypoints"].find(x => x["part"] == "leftEye");
    // let right = poses[0]["keypoints"].find(x => x["part"] == "rightEye");
    let right = poses[0]["keypoints"].find(x => x["part"] == "rightShoulder");

    if (left && right && poses[0]["score"] >= minPoseConfidence) {
      // console.log("score", poses[0]["score"]);
      ...
      // console.log(" ");
      if (diff_from_last > 2) {
        document.getElementById("display_vid").currentTime = to_time;
      }
      window.last_distance = dist_between;
